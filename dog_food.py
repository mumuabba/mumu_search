...
if not df.empty:
    ...
    with col_img:
        if os.path.exists("mumu.jpg"):
            try:
                ...
                st.image(rotated_img, width=120)
            except: st.write("🐶")
        else: st.write("🐶")
    with col_txt:
        st.markdown("## 무무 탐색기")
        st.caption("전국 반려동물 동반 식당 정보 서비스")

    st.divider()

    # [핵심] 지역 선택 필터를 메인 화면 상단에 배치! (사이드바 안씀)
    st.markdown("#### 📍 어디로 가시나요?")
    broad_regions = sorted([r for r in df["지역"].unique() if r not in ["미분류", "nan", "None"]])
    
    # 두 개의 필터를 가로로 배치 (모바일은 자동으로 세로로 정렬됨)
    c1, c2 = st.columns(2)
    with c1:
    ...
    if selected_broad != "지역을 선택하세요":
        ...
        st.success(f"🔍 {selected_broad} {selected_city if selected_city != '전체' else ''} 검색 결과: {len(final_df):,}건")
        ...
        st.info("위에서 지역을 선택하시면 맛집 리스트가 바로 나타납니다! 🐾")
...
st.markdown(f"""
    <div style="font-size: 0.85rem; color: #555; text-align: center; line-height: 1.8; background-color: #f8f9fa; padding: 25px; border-radius: 12px; border: 1px solid #eee;">
        ...
        공공데이터법에 의거하여 <b>식품의약품안전처</b>에서 제공하는 Open-API를 활용한 <b>비영리 목적의 사이트</b>임을 밝임을 밝힙니다.<br><br>
